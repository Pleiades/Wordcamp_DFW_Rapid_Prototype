<?php
/**
 * Based on the Genesis Featured Post widget (StudioPress) and Winning Agent Pro Communities widget (Carrie Dils).
 *
...
/**
 * Agent Focused Pro Featured Community widget class.
 *
 * @since 1.0.0
 */
class Agent_Focused_Pro_Featured_Community extends WP_Widget {
	...
	function __construct() {
		...
		$this->defaults = array(
			...
			'content_limit'           => '',
			'more_text'               => __( '[Read More...]', 'agentfocused' ),

		);

		$widget_ops = array(
			'classname'   => 'featured-content featured-community',
			'description' => __( 'Displays featured communities with thumbnails', 'agentfocused' ),
		);

		$control_ops = array(
			'id_base' => 'featured-community',
			'width'   => 505, // Keep same as genesis featured post widget.
			'height'  => 350,
		);

		parent::__construct( 'featured-community', __( 'Featured Communities', 'agentfocused' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
				...
				if ( $instance['show_image'] && $image )
					printf( '<a href="%s" title="%s" class="%s">%s</a>', get_permalink(), the_title_attribute( 'echo=0' ), 'wap-community-link', $image );
